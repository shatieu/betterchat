  ...
  async runner(context: AgentRuntimeContext, state: AgentState) {
    ...
    switch (context.phase) {
      ...
      case 'user_input': {
        ...
        // In other states, ignore user input and finish
      ...
      case 'llm_result': {
        ...
        // Manually add assistant message to state (fix for Runtime issue)
      ...
      case 'tool_result': {
        ...
        // If there are remaining tools, continue executing
...
async function main() {
  ...
  const runtime = new AgentRuntime(agent); // modelRuntime is now inside the Agent
